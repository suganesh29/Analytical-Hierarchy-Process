...
from PyQt5.QtWidgets import QTableWidget, QComboBox,QApplication, QMainWindow, QTableWidgetItem, QFileDialog,qApp, QAction,QStyledItemDelegate,QLineEdit,QWidget,qApp, QAction,QMessageBox
...
class comboUDclass(QComboBox):
    def __init__(self, parent):
        super().__init__(parent)
       
        self.setStyleSheet('background-color: #c4fbff;font-size: 25px;border-bottom: 1px solid')
        self.addItems(['9','8','7', '6', '5', '4','3','2','1','0.1111','0.1250','0.1430','0.1670','0.2000','0.2500','0.3333','0.5000'])
        #self.addItems(['9','8','7', '6', '5', '4','3','2','1'])
        self.currentIndexChanged.connect(self.getComboValue)
        
    def getComboValue(self):
        
        selectip=float(self.currentText())
...
class Ui_MainWindow(QWidget):
    ...
    def cc(self):
        global Csize
        Csize=self.lineEdit.text()
        print(Csize)
       
    def addRow(self):
        self.tableWidgetInput.insertRow(self.tableWidgetInput.rowCount()+0)  
        irowCount = self.tableWidgetInput.columnCount()        
        for i in range(irowCount):
            for j in range(iTabcol):
                if(i< j):
                    combo = comboUDclass(self)
                    self.tableWidgetInput.setCellWidget(i, j, combo)
                if(i==j):
                    self.tableWidgetInput.setItem(i, j, QTableWidgetItem("1")) 

    def addCol(self):    
        columnCount = self.tableWidgetInput.columnCount()
        self.tableWidgetInput.insertColumn(columnCount )
        icolumnCount = self.tableWidgetInput.columnCount()
        for i in range(iTabrow):
            for j in range(icolumnCount):
                if(i< j):
                    combo = comboUDclass(self)
                    self.tableWidgetInput.setCellWidget(i, j, combo) 
                if(i==j):
    ...
    def save_sheet(self):
        ...
        if path[0] != '':
            with open(path[0], 'w',newline='\n') as csv_file:
                ...
                writer.writerows(SavePWmatrix)


                ostr =['The Normalized Pair-Wise Matrix is  :']
                writer.writerow(ostr)
                writer.writerow("\n")
                writer.writerows(SaveNorPWmatrix)
                
                writer.writerow("\n")
                ostr0 =['Sum of Weight values are    :']
                writer.writerow(ostr0)
                writer.writerow(GeomeanWeightPrint)
                writer.writerow("\n")

                '''
                ostr1 =['The λ  value is    :']
                writer.writerow(ostr1)
                writer.writerow(MaxPrint)
                writer.writerow("\n")
                
                
                
                ostr2 =['The CI   values are   :']
                writer.writerow(ostr2)
                writer.writerow(CIPrint)
                writer.writerow("\n")

                ostr3 =['The CR   values are   :']
                writer.writerow(ostr3)
                writer.writerow(CRPrint)
                writer.writerow("\n") '''
    ...
    def submit(self):
        ...
        global EnteredData,listi ,isize,iTabrow,iTabcol,rowData,RI_dict,SavePWmatrix,SaveNorPWmatrix,inputArray,inputArray_2D,PWmatrix,PWmatrix_sum0,NorPWmatrix,NorPWmatrixRowsum,CriteriaWeight_arrPrint,GeomeanWeightPrint,PercenCriteriaWeight_arr,AWPrint,Max,Mx,MaxPrint,CI,CR,CIPrint,CRPrint,rw,rowss,res,result,Max,MaxPrint,CI,CIPrint,CR,CRPrint
        #print("Submitted the values")
        RI_dict = {1: 0, 2: 0, 3: 0.58, 4: 0.90, 5: 1.12, 6: 1.24, 7: 1.32, 8: 1.45, 9: 1.49}
        iTabrow = self.tableWidgetInput.rowCount()
        iTabcol = self.tableWidgetInput.columnCount()
        isize=iTabrow*iTabcol 
        rowData =[]   
        rowss=4     
        for r in range(iTabrow):
            for c in range(iTabcol):        
                widget = self.tableWidgetInput.cellWidget(r, c)
                
        
                if isinstance(widget, QComboBox):
                    current_value = float(widget.currentText())
                    #revsvalue =round(1/(current_value),4)  
                    rowData.append(float(current_value))
                if(r>c):
                    reversval = 0            #reciprocal(current_value)
                    rowData.append(float(reversval))
                if (r==c):
                    evencell = 1
                    rowData.append(float(evencell))

        

        EnteredData= len(rowData)
        if(isize==EnteredData):
            #print(EnteredData)
            print("Toatal number of elements: ",EnteredData)
            print(rowData)
            #print(type(rowData))
            ...
            A_arr = np.reshape(inputArray, (iTabrow, iTabcol))   ### Reshape the numpy array into 2D   
            PWmatrix1 = np.array(A_arr)
            tempPWmatrix=np.around(PWmatrix1,4)
            SizeA_arr=tempPWmatrix.shape[0]
            print('Pair-Wise Matrix:')  
            #print(type(tempPWmatrix))     
            #print(tabulate(tempPWmatrix , tablefmt="simple", floatfmt = ".3f"))
            

    ####################Change the half entered matrix into full inputmatrix##############################
            
            tri_upper_diag = np.triu(tempPWmatrix, k=0)
            #print(tabulate(tri_upper_diag , tablefmt="simple", floatfmt = ".3f"))
            tri_upper_no_diag = np.triu(tempPWmatrix, k=1)
            tri_upper_no_diag
            #print(tabulate(tri_upper_no_diag , tablefmt="simple", floatfmt = ".3f"))
            repir_triU_noDiag= (1/tri_upper_no_diag)
            #print(tabulate(repir_triU_noDiag , tablefmt="simple", floatfmt = ".3f")
            transpose= np.transpose(repir_triU_noDiag)
            #print(tabulate(transpose , tablefmt="simple", floatfmt = ".3f"))
            transpose[~np.isfinite(transpose)] = 0
            #print(tabulate(transpose , tablefmt="simple", floatfmt = ".3f"))
            PWmatrix= tri_upper_diag+transpose
            print(tabulate(PWmatrix , tablefmt="simple", floatfmt = ".4f"))
            print("\n")
            SavePWmatrix=np.around(PWmatrix,4)


    ########################calculate Geomean##########################################
            GmeanY= gmean(PWmatrix,axis = 1)
            GmeanYPrint= np.around(GmeanY,4)
            print("Geomean:", GmeanYPrint)
            GemanWeightSum=GmeanYPrint.sum(axis=0)
            GeomeanWeight=GmeanYPrint/ GemanWeightSum
            GeomeanWeightPrint= np.around(GeomeanWeight,4)

            print("\nGeomean Weight:",GeomeanWeightPrint)
            


            PWmatrix_sum0 = PWmatrix.sum(axis=0) # column sum
            NorPWmatrix = PWmatrix * GeomeanWeightPrint 
            NorPWmatrixRowsum=NorPWmatrix.sum(axis=1)
            print("\nNorPWmatrixRowsum :", NorPWmatrixRowsum)
            print('\nNormalized Pair- wise  matrix:')
            print(tabulate(NorPWmatrix , tablefmt="simple", floatfmt = ".4f"))
            SaveNorPWmatrix= np.around(NorPWmatrix,4)
            
            result = np.array(NorPWmatrixRowsum) / np.array(GeomeanWeightPrint)
            print("λ  Values :",result)
            res= np.around(result,3)
            ...
            print("\n")
            MaxPrint= np.around(Max,3)
            print('λMax: %s' % MaxPrint)
            print("\n")

            CI = (Max - rw) / (rw - 1)
            CIPrint= np.around(CI,3)
            print('CI: %s' % CIPrint)
            print("\n")
            
            CR = CI / RI_dict[rw]
            CRPrint= np.around(CR,3)
            print('CR: %s' % CRPrint)
           ...
           # W = NorPWmatrixRowsum.sum()
            ...
            aa=100
            x= CriteriaWeight_arr *aa
            PercenCriteriaWeight_arr= pd.DataFrame(x/float(np.sum(x))).applymap(lambda x: '{:.2%}'.format(x)).values
            CriteriaWeight_arrPrint=np.around(CriteriaWeight_arr,4)
            #print('\nCriteria weights:%s' % CriteriaWeight_arrPrint )
            #print(type (CriteriaWeight_arrPrint))
            #print('\nPercentage of Criteria weights:',PercenCriteriaWeight_arr )
            #print("\n")
            AW = []
            for a in PWmatrix :
                aa = a * GeomeanWeightPrint                 #CriteriaWeight_arr
                AW.append(aa.sum())
                AWPrint=np.around(AW,3)
        ...
        for i in range(iTabrow):
            for j in range(iTabcol):
                ...
                if it and it.text():
                    ...
                    self.textEdit4.clear()
                    self.textEdit5.clear() 
                    for row_number,rdata in enumerate(NorPWmatrix):   
                    ...
                    #self.textEdit1.insertPlainText(str(PercenCriteriaWeight_arr))# Percentage of Criteria Wights
                    self.textEdit2.insertPlainText(str(GeomeanWeightPrint))#  Weighted Sum
                    self.textEdit3.insertPlainText(str(MaxPrint))#  λ Max
                    self.textEdit4.insertPlainText(str(CIPrint))#CIPrint CI
                    #self.textEdit5.insertPlainText(str(CRPrint))#CRPrint CR
      #aaffff              
...
"    border-bottom-color: #55ffff; /* same as the pane color */\n"
...
################################INPUT  FRAME  ###################################################################################################################################################################################################
        ...
        '''self.labelN = QtWidgets.QLabel(self.frame1)
        self.labelN.setGeometry(QtCore.QRect(500, 10, 150, 30))
        ...
        self.lineEdit.setGeometry(QtCore.QRect(700, 10, 150, 30))
        self.lineEdit.setObjectName("lineEdit")

        self.pushButtonC = QtWidgets.QPushButton(self.frame1)
        self.pushButtonC.setObjectName("pushButtonC")
        self.pushButtonC.setGeometry(QtCore.QRect(900, 10, 150, 30))
        self.pushButtonC.clicked.connect(self.cc)'''
        ...
        self.tableWidgetInput.setColumnCount(4)
        self.tableWidgetInput.setRowCount(4)
        delegate = NumericDelegate(self.tableWidgetInput)
        self.tableWidgetInput.setItemDelegate(delegate)
       
        tsize=self.tableWidgetInput.rowCount()
        for i in range(tsize):
            for j in range(tsize):
                ...
                if(i>j) :
                    self.tableWidgetInput.setEditTriggers(QtWidgets.QTableWidget.NoEditTriggers)

                    #self.tableWidgetInput.setItem(i, j, QTableWidgetItem("0"))
        #self.TableWidget.setItemDelegate(FloatDelegate(3))
        #self.tableWidgetInput.cellChanged.connect(self.cellReverse)
        #self.tableWidgetInput.cellEntered.connect(self.cellcheck)
...
################################OUTPUT FRAME  ##################################################################################################################################################################################################
        ...
        self.tableWidgetOutput.setColumnCount(8)
        self.tableWidgetOutput.setRowCount(8)
    ...
    def retranslateUi(self, MainWindow):
        ...
        #self.labelN.setText(_translate("MainWindow", "Enter the Criteria Size"))
        #self.pushButtonC.setText(_translate("MainWindow", "Submit"))
        ...
        self.label_33.setText(_translate("MainWindow", "The CI CR  Data's"))
        ...
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab1), _translate("MainWindow", "% C.W"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab2), _translate("MainWindow", "W.Sum"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab3), _translate("MainWindow", "λMax "))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab4), _translate("MainWindow", "CI "))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab5), _translate("MainWindow", "CR "))
